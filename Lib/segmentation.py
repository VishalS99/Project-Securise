...
def sort_clockwise(points):
    ...
    start = coords[0]
    rest = coords[1:]
    rest = sorted(rest, key=lambda coord: angle_with_start(coord, start[0]), reverse=True)
    rest.insert(0, start)
    return rest
...
def detection(sorted_contours, image, bound, j):
    ...
    for cnt in sorted_contours:
        ...
        white = [255,255,255]
        roi = cv2.copyMakeBorder(roi, 5, 5, 5, 5, cv2.BORDER_CONSTANT, value= white)
...
def character_segmentation(image,j):
    ...
    image_crop = border(dilation, image, ratio, j)
    ...
    print(bound)
    return detection(sorted_contours, image_crop, bound,j)
